...
```mermaid
graph TD
    Hero["Hero Video / Image (Dark Overlay)"] --> Sec1["Section 1: Annual Gala 2026 Recap Spotlight (bg-off-black, crafted gold seam)"]
    Sec1 --> Sec2["Section 2: Relive the Summer Recap Band (Light Cream: Graduation + Sweat Fest cards)"]
    Sec2 --> Sec3["Section 3: ABC7 Windy City Weekend Spotlight (bg-off-black, click-to-load player)"]
    Sec3 --> Sec4["Section 4: Monday Miles Tee Merch (Light)"]
    Sec4 --> Sec5["Section 5: 40 Under 40 (bg-off-black)"]
    Sec5 --> Sec6["Section 6: Monday Miles Community Runs (Light Sand)"]
    Sec6 --> Sec7["Section 7: A New Chapter (relocation) (bg-off-black)"]
    Sec7 --> Sec8["Section 8: Events Carousel (Light Sand)"]
    Sec8 --> Sec9["Section 9: Instagram (bg-off-black)"]
    Sec9 --> Sec10["Section 10: In The Press (Light)"]
```
...
> [!NOTE]
> **July 2026: Haus of LSP Summer Renaissance added (a 7th dark section).** Summer Renaissance (Sat Aug 1, 6-11 PM, Pilsen; see Option G below for the identity) is chronologically the soonest event, but its identity is a **dark** nightlife collage, and a dark section cannot sit against the dark hero. So Sweat Fest keeps the lead light slot and Summer Renaissance takes the **first dark slot**, directly after it. Adding a 7th dark section to a 6-dark/6-light page makes strict alternation impossible without a rotation, so every dark section below it was rotated **down one light-slot** (five adjacent pair swaps executed as a marker-based block reorder): Hero dark -> Sweat Fest light -> **Summer Renaissance dark** -> Merch Drop light -> Gala dark -> Free Day Promo light -> WGN9 dark -> Monday Miles light -> 40 Under 40 dark -> Events Carousel light -> Grand Opening dark -> Press light -> Instagram dark. Perfect alternation holds with zero same-theme adjacencies; the page now ends dark (Instagram) before the footer. Media zig-zag re-audit after the rotation: Sweat Fest poster left -> **Summer Renaissance collage right** -> Merch tee left -> Gala photo right -> **WGN9 video flipped to the left** (`lg:order-*` on its two grid children) -> Monday Miles right -> 40 Under 40 left -> **Grand Opening image flipped to the right** (swapped `lg:order-*`). Free Day, the Events carousel, and Instagram remain outside the zig-zag. The carousel gained a Summer Renaissance card in first position (soonest event). The mermaid diagram above reflects the new order.
>
> On the standalone events page, the dark Summer Renaissance featured section (`id="summer-renaissance"`, `scroll-mt-28`) was inserted **between the light page header and the light Sweat Fest section**, which both leads with the soonest event and dissolves that page's long-standing header -> Sweat Fest light/light adjacency: Page Header (light) -> Summer Renaissance (dark) -> Sweat Fest (light) -> Gala (dark) -> EventsList (light) -> Monday Miles (light sand, pre-existing) -> Stay Connected (light).

> [!NOTE]
> **August 2026: Summer Renaissance + Free Day retired, Graduation 2026 leads.** Summer Renaissance (passed Aug 1) and the free-class-days promo (campaign ended) were retired from every placement: homepage sections, carousel card, events featured section, links cards, and both highlight bubbles (the `/hausoflsp` page persists, per the Pride precedent). The **Graduation 2026** recap (Option H below) then took the LEAD slot directly after the hero at the user's direction, in its DARK candlelit-rose identity, which makes the hero -> graduation junction the page's one intentional same-theme adjacency: it is carried by the **"candlelit horizon" seam** (a static gold+orchid glow wash across the section's top edge plus breathing candle pools and rising bokeh dots, all on the slow `gala-ambient` breathe), and by the palette contrast between the hero's neutral black overlay and the section's warm rose-ink. Final homepage order: Hero D -> **Graduation D (crafted seam)** -> Sweat Fest L -> Gala D -> Merch L -> WGN9 D -> Monday Miles L -> 40 Under 40 D -> Events Carousel L -> Grand Opening D -> Press L -> Instagram D; every junction below the seam strictly alternates. Desktop media zig-zag: Graduation RIGHT -> Sweat Fest LEFT -> Gala RIGHT -> Merch LEFT -> WGN9 RIGHT -> Monday Miles LEFT -> 40 Under 40 RIGHT -> Grand Opening LEFT (carousel + Instagram outside). Note: the v4 Sweat Fest section's top strip is the checkerboard tile strip (the sunrise `h-1` gradient belonged to the pre-checkerboard identity). The events page runs Header L -> **Graduation recap D** (`id="graduation-2026"`) -> Sweat Fest L -> Gala D -> EventsList L -> Monday Miles L -> Stay Connected L. The events carousel leads with the "Graduation 2026 Gallery" card; links feature cards run Graduation -> Sweat Fest -> Gala (chronological: Aug 7 recap, Aug 22, Sep 25) with a "Grad 2026" bubble first in the highlights bar and the Free Classes bubble/promo removed.

> [!NOTE]
> **September 2026: the gala takes the lead.** Full-evening (Benefactor) tickets close **Tuesday, September 15 at 11:59 PM Chicago** and the gala itself is **Friday, September 25**, so for the closing stretch the Annual Gala was promoted out of the middle of the page into the homepage marquee slot directly under the hero. That makes the hero -> gala junction the page's one intentional same-theme adjacency (it replaces the retired Graduation "candlelit horizon" seam), and it is carried by a crafted **gold seam**: a `.gala-foil-rule` hairline on the very top edge, a full-width **static** gold wash pooling up into the hero (`radial-gradient` at `rgba(255,189,89,0.18)`, `h-40`, no animation), two slow-breathing `.gala-ambient` blobs (gold left, mauve right, staggered `animation-delay`), the `.gala-lattice` diamond weave at 5% over the whole section, and a sparse desktop-only field of 12 floating gold motes (`hidden lg:block`, motion-safe). Final homepage order: Hero D -> **Gala D (crafted gold seam)** -> "Relive the summer" recap band L -> WGN9 D -> Monday Miles Tee L -> 40 Under 40 D -> Monday Miles L -> Grand Opening D -> Events carousel L -> Instagram D -> Press L; every junction below the seam strictly alternates. Desktop media zig-zag: Gala photo LEFT -> WGN9 video RIGHT -> Tee LEFT -> 40 Under 40 flyer RIGHT -> Monday Miles visual LEFT -> **Grand Opening image flipped to the RIGHT** (its two `lg:order-*` values swapped: copy `lg:order-1`, image `lg:order-2`). The recap band, carousel, Instagram, and Press sit outside the zig-zag. The mermaid diagram above reflects this order. In the same pass the Studio / Grand Opening section was rewritten as **"A New Chapter"** for the studio relocation: daily classes at 949 W 16th St run through September 25, pop-up classes and community programming follow around the neighborhood, the new Pilsen home opens early 2027, and a second ghost CTA sends people to `/donate` alongside the existing schedule button; its slot (7), its dark `bg-off-black` theme, and its RIGHT media side are all unchanged. About's "Our Studio Is Open" and "Our New Home" sections became **"A New Chapter"** and **"Our Next Home"** (same relocation copy as the homepage: classes through Sep 25, pop-up after, new Pilsen home early 2027, donate CTA), and the 2025 Impact Wrapped "New Home" milestone was reworded to past tense as **"First Studio"**.
>
> **Retirements.** The **YTT '26 Trio community classes** (series ended Aug 31) came off every surface: the homepage companion band, the events-page trio strip, the `/schedule` section, and the links card. **Graduation 2026 and Sweat Fest '26 were demoted from full spotlights to a single light two-card "Relive the summer" recap band**, where each card is still an ambassador of its own identity per the Option H exception below: Graduation keeps the candlelit-rose room (Fraunces, `#f0c57c` gold, candle-bokeh dots, cream keepsake mat, never `.gala-foil` or `accent-gold`) and Sweat Fest keeps the v4 checkerboard print kit (honeydew field, `#123f36` ink, `sf-*` utilities, hard magenta offset shadow). The **Sweat Fest merch drop section and `<MerchLightbox />` were removed from the homepage** (that drop sold on site on Aug 22 only), along with the `.home-merch-*` styles and the now-unused imports. The light-band copy of `GalaTicketDeadline` also came off the homepage: the gala spotlight carries the inline module instead, so the page makes one ask.
>
> **Carousel and hero pill.** The events carousel heading became **"Events at LSP"** (it holds recaps now, not only upcoming events) and its four cards run **Gala -> Monday Miles -> Graduation 2026 Gallery -> Sweat Fest**. The gala card's three top badges became two: "Black tie" and a gold `data-gala-days-left` chip prefixed by a small rotated gold diamond. The hero pill keeps the gala announcement, swaps its `animate-ping` dot for a static `h-1.5 w-1.5 rotate-45 bg-accent-gold` diamond, and ends in a live `data-gala-closes-in` span, so it reads "Annual Gala · Sep 25 · closes in 4 days" and relabels itself as the phase changes (see the `GalaTicketDeadline` contract in 8B).

> [!NOTE]
> **September 2026: the same refresh on `/events`, `/schedule`, and `/links`.** **Events page** order: Page header L -> **Annual Gala D leading the page** (`id="noche-inolvidable-gala"` kept, with the eyebrow, foil H2, a `galaEvening` chip row, and an inline `GalaTicketDeadline`) -> Sweat Fest recap L -> Graduation 2026 recap D **with its YTT Trio strip deleted** (the trio slugs stay in `featuredSlugs` so the Supabase island never resurfaces them) -> `<EventsList />` -> Monday Miles -> Stay Connected. The old `events_top` deadline band was removed: the gala section leads now, so a second block would be spam. **Schedule page**: the redesigned `GalaTicketDeadline` banner (`context="schedule"`) sits at the very top above the policy strip, and the entire `#ytt-trio` section, its `.trio-*` styles, the `trio-pulse-gold` keyframes, and its script were deleted. **Links page**: the standalone bilingual deadline block above the highlights bar was removed; the highlights bar now runs **Gala 2026 -> Reader -> Grad 2026 -> Sweat Fest -> WGN -> YTT 2025 -> Therapy -> App** and the feature cards run **Gala -> Reader -> Graduation -> Sweat Fest**; the `#trio-card` is gone with its `.trio-next-tag` / `.trio-row` styles, its script block, and the `ytt_trio_ticket_click` branch. The gala card carries an inline **bilingual** `GalaTicketDeadline` between the price tiles and the closing `statusLabel` line, and its old bottom "Get Your Full-Night Gala Ticket" CTA was deleted so the card makes one ask; the "On Sale Now / A la Venta" chip became a gold `data-gala-days-left` chip using the standard `.lang-en` / `.lang-es` pair. Card `animation-delay` values were re-staggered to DOM order (80, 100, 115, 130 ms).

> [!NOTE]
> **September 26, 2026: ABC7 "Windy City Weekend" feature (the "on air" identity).** ABC7 Chicago's Windy City Weekend had Junior Board President Yari Jurado and incoming Vice President Savannah Alvarez on set on gala day (Sep 25): the mission, the three pillars, the search for the next studio, then a short yoga lesson with the hosts. The ABC7 article (`abc7chicago.com/post/latina-sweat-project-joins-windy-city-weekend-…/19872532/`) embeds the **full 23:33 episode** (video 19871878), not a clip; our segment starts at **19:29** and ABC7's embed URL accepts no start-time parameter (tested `&t=`, `&start=`, `#t=`), so every surface shows a "starts at 19:29" hint. **Rules:** embed only through the official `https://abc7chicago.com/video/embed/?pid=19871878` URL, never hotlink or self-host ABC7's mp4/HLS, always link the article. **Single source of truth:** `src/data/abc7WindyCity.js` (every EN/ES string, URLs, guests, chips, CTA labels, the abc blue `#0058f0` / ink `#202030` tokens); the press entry in `src/data/press.ts` reads from it and carries `key: "abc7-windy-city"` + `logo` (entries with `key` can be filtered out of grids where they have their own hero). **Player:** `src/components/Abc7Segment.astro` is a click-to-load facade (LSP-owned gala photo dimmed under scanlines, the ABC7 bug tile, a play button, the hint) that injects the iframe only on play, supports several instances per page, `lang="both"` for the links toggle, and outside triggers via `data-abc7-trigger="<player id>"`; it also always renders a "Trouble playing? Watch on ABC7" fallback link. **Identity:** design.md Option A dark section with an abc-blue top accent line, an abc-blue glow behind the player and gold behind the copy, an "On air · ABC7 Chicago" chip with a motion-safe red pulsing dot next to the logo in a small white rounded tile (the logo's dark "abc" circle disappears on dark backgrounds otherwise), Rubik black headline with the pink-to-gold gradient span, abc-blue primary CTA and a white ghost CTA. **Placements:** (1) `/press`: a full-bleed on-air hero (`#abc7-windy-city`, player first on mobile, right on desktop) between the page hero and the Reader clipping, the ABC7 card filtered out of the grid; the Reader clipping is untouched. (2) Homepage: the ABC7 spotlight **replaced the WGN9 spotlight in slot 4** (same dark theme, copy `lg:col-span-5` left, player `lg:col-span-7` RIGHT, natural DOM order), so the light/dark cadence and the media zig-zag are unchanged; the WGN9 spotlight is retired from the homepage but keeps its press card and `/wgn9-around-town`. The spotlight uses `overflow-clip` (not `overflow-hidden`) so a scroll-to-player call cannot scroll the section's own box. The gala recap spotlight gained a gold "Watch Yari and Savannah on ABC7" link and A New Chapter a "Hear Yari and Savannah tell the story on ABC7." sentence, both anchoring to `#abc7-windy-city`; press-grid cards now show an outlet logo tile when the entry has `logo`. (3) `/links`: the `#abc7-card` feature card sits directly after `#gala-card` (ink shell, abc-blue-to-gold top strip, "On TV / En TV" chip, bilingual byline, headline, short blurb, the player with `lang="both"`, hint, full-width abc-blue "Read on ABC7 / Leer en ABC7" bar); the "📺 ABC7" bubble is second in the highlights bar (Gala 2026 -> ABC7 -> Reader -> Grad 2026 -> Sweat Fest -> WGN -> YTT 2025 -> Therapy -> App) and the feature cards were re-staggered to 80, 95, 110, 125, 140 ms. (4) `/gala` recap: a compact `#abc7` band after `#gracias` (gold eyebrow with the logo tile, `font-display` H2, short blurb, inline player, ghost "Read on ABC7"). Tracking: `press_abc7_click` (article/video links) and `press_abc7_play` (facade activation) with contexts `press_abc7_hero`, `home_abc7_spotlight`, `home_gala_spotlight`, `home_new_chapter`, `links_abc7_card`, `gala_recap_abc7`.

> [!NOTE]
> **Post-gala recap (Sep 26, 2026).** The Annual Gala happened Friday, September 25, 2026 at MCA Chicago, so every gala placement outside `/gala` flipped from "sell tickets" to "recap: thank you, photos, what the night raised", following the Sweat Fest recap precedent (commit 3eccf3c). **Data:** `galaTeaser.isPast = true`, `statusLabel` "That's a wrap", `lateNightStatusLabel` "Thank you for a night at the MCA", `recapPath` `/gala`, `recapLabel` "Relive the night", plus a new `galaRecapCard` (eyebrow, heading "Gracias, comunidad", date line, EN blurb + one ES line, CTA "See the photos", three aggregate stats: $25,182.50 raised on the night, 40 paddles raised, 422 photos). `galaTickets`, `galaEvening`, and `galaTicketDeadline` stay exported as historical data but nothing public renders prices; `GalaTicketDeadline` resolves to its `past` phase and hides itself (so the `/schedule` band disappears on its own). **Homepage:** the lead slot, its dark theme, the crafted gold seam, the lattice, the ambient blobs, and the mote field are all unchanged, so the documented cadence (Hero D -> Gala D seam -> recap band L -> ...) is untouched; only the content became the recap: the local recap photo LEFT in `GalaPhotoFrame` (badge "Sep 25 · MCA Chicago"), copy RIGHT (eyebrow, foil H2 "Gracias, comunidad", the blurb, a three-stat row with Playfair numerals and gold small-caps labels), ONE gold CTA "See the photos" -> `/gala#photos` (`gala_recap_click` / `home_gala_recap`) and a ghost "Support the new home" -> `/donate`. Removed: the evening timeline (desktop track + mobile chips), the inline `GalaTicketDeadline`, the impact strip, "One night that funds the movement", and every ticket CTA and `data-gala-*` hook. The hero pill now reads "Annual Gala 2026 · Gracias, comunidad · see the photos" -> `/gala#photos` with no live spans, and the carousel card shows "Recap" + "422 photos" badges with a "Relive the night" CTA to `/gala`. **Events:** the leading dark section keeps `id="noche-inolvidable-gala"` for old anchors and becomes the recap block (eyebrow "Recap · Annual Gala 2026", foil H2, date/venue, blurb, three stats, gold "Relive the night" CTA + a text link to the Flickr album); the chip row, deadline module, and sponsor tease are gone, and the Header L -> Gala D -> Sweat Fest L cadence holds. **Links:** see the `/links` note in section 3. **Header:** the plain "Gala" nav link (desktop and mobile) already carried no date or on-sale badge, so it simply stays and points to `/gala`. **`/lspgala`** (printed on 150 paddles and the program, must keep working): post-gala state: H1 "Gracias, comunidad." with "The Annual Gala was Friday, September 25 at the MCA.", Give stays first, a "Relive the night · Photos and results" button to `/gala`, the program PDF link stays, the run of show is retitled "The evening as it happened", and the silent-auction button, the `/gala/live` link, and the `HubLive` island (stale live total; usage commented out, component untouched) were dropped. **`/silentauction`** now redirects to `/gala#auction` (the auction closed on the night). **The `/gala` page is now the recap of record**, in this order: hero, thank-you, the numbers, the paddle-raise constellation, the silent auction, the honors, the photos in four chapters (cocktail hour, dinner and honors, gallery and open bar, the fashion show and the dance floor), gratitude, and give. Its data lives in `src/data/galaRecap2026.js` (thank-you copy, `galaResults`, auction packages, honors, voices, sponsors) and `src/data/galaGallery2026.js` (all 422 Flickr stems + `galaAlbumChapters`). **Rule: results render as aggregates only** (totals, gift counts, levels, packages claimed); never donor, bidder, or guest names, and never caption people in photos. Homepage, events, and links card photos are local webp files in `public/images/gala/2026/recap-home/` (album #69, dinner under the gold lanterns; 1200w landscape + an 800x1000 4:5 crop), not hotlinked. `/gala` stays in the sitemap; `/lspgala` and `/silentauction` stay excluded.
...
Use this for celebration events or cultural observances where the moment carries its own palette but the site still needs LSP's warm, premium look. It is **currently carried by Celebrating Pride** (standalone `/pride` page + homepage lead spotlight + homepage Pride Teacher Spotlight + events featured section + links card, final Pride weekend, June 2026). It was previously carried by **Honoring Juneteenth** (passed June 21, 2026, retired from all placements; the standalone `/juneteenth` page and its `public/images/juneteenth/` assets were deleted) and before that by **Pride in the Park** (passed June 14, 2026). Keep it on hand for the next moment that ships its own palette.
...
**Honoring Juneteenth (June 2026, retired):** the standalone page (`src/pages/juneteenth.astro`) and its assets (`public/images/juneteenth/`) were deleted once the holiday passed and Pride took over the placements. Reusable lessons kept from that build:
- **Palette:** Pan-African red (`#e4392b`) + green (`#1e9e5a`), grounded with off-black and accent gold; thin red -> gold -> green top strips and red (Friday) / green (Saturday) day chips. On cream, run the heading gradient red -> Kids Clay (`#c7602d`) -> green (gold is too low-contrast for text on cream).
- **Standalone page pattern:** a richer **dark** variant of the celebration palette (the same shape `/pride` now uses), with a `#ways-to-show-up`-style anchor (`scroll-mt-28`) and a light closing tie-in to the schedule.
- **Mixed flyers + photos:** frame event images with `aspect-[3/2]` + `object-contain` on a subtle dark panel so flyer text is never cropped while photos still fill the frame.
- **Dual-cause banner:** `src/components/PrideTheme.astro` can run the banner as **two cross-fading slides** so overlapping observances each get an equal turn (it carried Pride + Juneteenth in mid-June 2026). Keep observance banners date-gated and self-removing.

**Celebrating Pride live implementation (final weekend, June 27-28, 2026):**
- **Palette:** the full six-stripe rainbow, used only for **thin accents** (top strips, the `lsp-pride-bar` rule, day chips, badges) so the page never goes one-note. Three shared helpers live in `src/styles/global.css` so the homepage and events placements share one source of truth: `.lsp-pride-bar` (rainbow hairline), `.lsp-pride-text-bright` (bright clipped-text rainbow for text on **dark** backgrounds, e.g. an off-black pill), and `.lsp-pride-text-deep` (deeper, saturated red->orange->green->blue->violet for headings on **cream**, where pale stops like yellow are illegible). The standalone page defines its own equivalents (`.pride-bar` / `.pride-text`) in its scoped block.
- **Standalone page** (`src/pages/pride.astro`): the richer **dark** variant (`bg-off-black`, white text). Hero with the Pride Month honoring statement, a honoring-statement card, a **"Amazing ways to show up"** grid of four Chicago weekend events (Proud to Run, Pride Pilates Party, Chicago Pride Parade, Back Lot Bash; each card carries a single Pride accent so the set spans the flag, with `#celebrate` anchor), the **Teacher Spotlight** swipeable decks (`#spotlight` anchor), and a closing LSP-schedule tie-in. Event facts were confirmed against each organizer's page and the section carries a confirm-before-you-go note.
- **Swipeable spotlight decks** (the marquee feature): two queer teachers (Luz Rodriguez she/her/ella, lizlates she/her), each rendered as a native scroll-snap card deck (`[data-deck]`) of the marketing team's designed Instagram Pride cards, paired with native context (name, pronouns chip, practice, bio, pull-quote, schedule chip, Book CTA) in an alternating two-column layout (`lg:order-*`). Native `scroll-snap-type: x mandatory` handles touch/trackpad; an inline script wires prev/next arrows, dot pagination (one dot per card, `i * track.clientWidth` paging), keyboard arrows on the focusable track, and disables arrows at the ends. Each card image carries full alt text transcribing its Q&A so the content is available to assistive tech and search.
- **Spotlight assets** (`public/images/pridespotlight/1.jpg` - `10.jpg`): the 10 supplied 1080x1350 PNG carousel cards (1-5 Luz, 6-10 lizlates) were converted to JPEG at 1000px wide / quality 82 via `sips` (6.5MB -> 2.1MB; no `cwebp`/libwebp available locally) and the PNGs removed, since the cards are photo+gradient composites that compress poorly as PNG. All deck images are `loading="lazy"` `decoding="async"`. The Luz/lizlates **cover** cards (`1.jpg`, `6.jpg`) double as the teaser media on the homepage and events Pride sections.
- **Homepage** (`src/pages/index.astro`): lead **Celebrating Pride** light spotlight (replacing Juneteenth; copy left + "this weekend" text list right, outside the zig-zag, CTA to `${base}pride`) and a **Pride Teacher Spotlight** light section (replacing Father's Day; the two cover cards on the left to preserve the zig-zag, two quote rows, CTA to `${base}pride#spotlight`). The events carousel's Father's Day card became a rainbow "Celebrate Pride with LSP" card.
- **Events page** (`src/pages/events.astro`): a **Celebrating Pride** light featured section (`id="pride-at-lsp"`, `scroll-mt-28`) replacing Father's Day, with the two cover cards, a 2x2 grid of the four weekend events, and CTAs to `/pride` and the schedule.
- **Links page** (`src/pages/links.astro`): first featured card, `color: "pride"` (rainbow badge, already-defined treatment), `icon: "heart"`, badge `This Weekend` / `Este Finde`, internal `target: "_self"` link to `${base}pride`. Replaced the retired Juneteenth and Father's Day cards.
- **Nav + mobile menu** (`src/components/Header.astro`): an elegant rainbow **"Pride 🏳️‍🌈"** link in the desktop nav (between Events and Impact; `.pride-nav-text` clipped-rainbow word) and a filled-rainbow **"Celebrate Pride"** card in the mobile menu (after Home). Both are gated behind `:global(html.pride-2026)` (set by `PrideTheme.astro` until the Pride end date), so the rainbow nav items **self-hide after June 30** with no manual takedown, even though the `/pride` page persists.
- **Banner** (`src/components/PrideTheme.astro`): the single remaining Pride slide links to `${base}pride` with a "Celebrate this weekend ->" CTA; the dual Juneteenth slide + its window-removal logic were deleted once Juneteenth passed.

#### Option E: Warm Sunset Event Theme (Father's Day)
Use this warm-light variant of Option B for community-celebration classes that are booked through the studio scheduler (not Zeffy). The first implementation was **Father's Day at LSP** (passed June 21, 2026, now retired from all placements: its homepage spotlight and events featured section became Pride sections, and its homepage carousel and links cards were swapped or removed). Keep this pattern on hand for the next studio-booked community-celebration class.
...
- **Past-state poster copy:** `src/data/sweatFestPoster.js` reads `sweatFest.isPast` **at call time**, so the poster's ticket line flipped on its own once the festival passed: it now reads **"Relive the day on our website"** (was "Tickets available on our website") above the same underlined `latinasweatproject.com/sweatfest` URL. Re-rendering the poster after an event needs no edit, and the flag must stay read lazily so it is never baked in at import time.
- **Single source of truth:** `src/data/sweatFest.js` drives every surface (homepage teaser, events featured section, links card, `/sweatfest` page, graphics). Tickets are live through the shared `ticketsUrl`; venue remains **TBD** (`venueName: null`). Prices, times, and the day-arc copy come from the planning doc and should stay exact.
- **Volunteer application (`/sweatfest#volunteer`, July 2026):** The page carries a collapsed, LSP-styled native application between the day schedule and tickets. It posts directly to the public Google Form through a hidden response frame, so applicants never see an embedded Google UI; `sweatFestVolunteer` in `src/data/sweatFest.js` is the source of truth for the responder URL, entry ids, exact checkbox values, and acknowledgements. The copy must keep all three terms explicit: applying does not guarantee placement, the LSP team approves volunteers, and selected volunteers must attend required pre-event training. Availability is split into 2-hour shifts from 6:00 AM setup through 10:00 PM cleanup, and applicants are prompted to select every shift they can genuinely attend. Keep the direct Google Form fallback link visible and never rename a stored checkbox value on only one side—Google matches posted checkbox responses by exact option string.

#### Option G: Nightlife Collage Theme (Haus of LSP)
Use this **dark-first** identity for Haus of LSP, LSP's queer-centered event series, and its nights (first carried by **Summer Renaissance**, Sat Aug 1, 2026, 6-11 PM, LSP Studio · 949 W 16th St, Pilsen; all proceeds fund the **LSP Queer Scholarship**). The look is the series' printed flyer rebuilt in DOM: a queer nightlife scrapbook, deliberately the opposite of both the gala's black-tie foil and Sweat Fest's daylight print kit.

- **Backgrounds:** a dark plum room, `linear-gradient` from maroon `#3a1626` to plum `#241019` (tokens in `hausColors`, `src/data/hausOfLsp.js`), with violet `#a06bd8` and pink `#ff6fb5` radial glows. Never `bg-off-black` flat: the warmth of the plum is what separates a Haus section from a gala section at a glance.
- **Palette:** hot pink `#ff6fb5` (marker ink, CTAs), violet `#a06bd8` (photo wash, offset shadows), mint `#b8ecd2` (sticker trim + the scholarship callout, used sparingly), notebook paper `#f5f1e6`.
- **Typography:** `font-marker` (Permanent Marker, loaded in `Layout.astro`, declared in the `@theme` block) for display words and the handwritten schedule; body copy stays Rubik. Signature move: a two-tone marker title, pink glow word + `.haus-chrome` liquid-metal word.
- **Motifs (shared utilities in `global.css`):** `.haus-glow` (pink party-light text glow), `.haus-chrome` (chrome clipped-text), `.haus-tape` (clear packing tape), `.haus-note` (torn ruled notebook page, carries the "qué va a pasar?" schedule), `.haus-sticker` (white die-cut border), `.haus-halftone` (zine dot overlay), `.haus-disco` (disco ball, SVG supplied inline from `hausDiscoUri`), `.haus-btn` (hand-inked button, violet offset shadow, presses flat).
- **3D disco ball + light effects (`/hausoflsp` hero):** `HausDiscoBall.astro` renders a three.js mirror ball (instanced silver facets, RoomEnvironment reflections, orbiting pink/violet lights, glint sprites, volumetric beam cones) over the flat `.haus-disco` SVG, which remains the no-WebGL fallback. The page also carries `.haus-room-lights`, a fixed screen-blended overlay of tiled soft light speckles drifting across every section. **Photosensitivity rules for all Haus motion (keep these on any future effect):** no strobe or flicker anywhere; beam opacity breathes on 16-25s sine cycles capped at ~0.19; the speckle layers only translate (90s/150s per tile) and never animate opacity; glints are small, sparse, and staggered; `prefers-reduced-motion` gets a single static 3D frame, a still speckle field, and no sway.
- **Forbidden on Haus surfaces:** `.gala-foil`, `font-display`/Playfair, `font-slab`/Ultra, and the papel picado strip: each identity keeps its signature moves exclusive.
- **Single source of truth:** `src/data/hausOfLsp.js` drives every surface (homepage spotlight, events featured section, links card, `/hausoflsp` page, the carousel card). Ticketing/entry details are **TBD** (`ticketsUrl: null`, `ticketsOnSale: false`); until they flip, every primary CTA routes to the `/hausoflsp` page (or Instagram on the page itself) and the status stamp reads "Entry details announced soon". When entry goes live, set `ticketsUrl` + flip `ticketsOnSale` in that one file and every CTA becomes a Get Tickets link with `haus_ticket_click` tracking.
- **Series clarity:** every surface renders "Haus of LSP presents" above the event title and states the Queer Scholarship cause line, so the series brand and the fundraising purpose always travel with the event name.
- **Artist/performer sign-up (`/hausoflsp#artists`):** the Google Form ("Latina Sweat Project: Art Exhibit/Open Mic") restyled as an on-theme "sign-up sheet" on the torn note: marker labels, dashed handwriting blanks, sticker-chip radios. Entry ids live in `hausArtistForm` (`src/data/hausOfLsp.js`). **The form cannot be posted from the site**: its final question is a required Drive file upload (2+ art photos), which forces Google sign-in and has no public endpoint. So submit builds a pre-filled `viewform?usp=pp_url` URL (dates split into `entry.N_year/_month/_day`) and opens the real form for the upload + submit. Radio values must match Google's option strings exactly. The homepage Haus band carries a marker "P.S." link to the anchor.
- The first printed flyer's "949 W 19th St" was a typo: the venue is the LSP Studio, 949 W **16th** St. The site renders the corrected address everywhere, and the marketing team shipped corrected flyer art (2026-07-13) so the scans (`public/images/hausoflsp1.jpg` / `hausoflsp2.jpg`, converted from supplied PNGs to 1000px JPEGs per the pride-spotlight precedent) now match.

#### Option H: Candlelit Rose (Graduation / Milestone Recap Theme)
Use this **dark-warm** identity for milestone recaps of ceremonial community moments: first carried by **Graduation 2026** (YTT Class of 2025 + 2026, Friday Aug 7, 2026, LSP Studio). The look is derived from the night's own photography (a candlelit room washed in rose/orchid party light), deliberately distinct from the gala's navy-and-foil luxe, Sweat Fest's daylight print kit, and Haus's neon collage.

- **Backgrounds:** deep rose-ink gradients `#2b1520 -> #1e1216` (never flat black, never navy). Paired radial glows: candle gold `rgba(215,155,71,~0.2)` + orchid rose `rgba(224,140,184,~0.14)`.
- **Palette:** soft candle gold `#f0c57c` for accents/captions/kickers; deeper gold `#d79b47` reserved for CTA fills only; orchid rose `#e08cb8` for glows and sparing accent text; cream `#f6efe3` for all body/display text.
- **Typography:** display face is **Fraunces** (loaded globally in `Layout.astro`; weights ~450-650, italics for Spanish echoes and quotes). Rubik stays for labels/badges/body. **Playfair / `font-display` is forbidden on graduation surfaces** (gala-exclusive).
- **Elements:** NO thin hairline rules and NO gradient bars anywhere (those are gala signature moves). Dividers/ornaments are **candle-bokeh dot rows** (3-5 small glowing gold/rose dots at varying size/opacity), soft glow orbs, and generously rounded **cream keepsake photo mats** (large concentric radii). Pill badges are fine. A gold **class seal** (concentric rings + class years) is the ceremonial motif; the `/graduation` page also carries a one-off **lineage thread** SVG (gold-to-rose curve connecting cohort -> nearly 100 -> hundreds more).
- **Copy voice:** warm invitation, not stat-dumping (the "307 photos" count appears at most once per surface); bilingual EN with short ES echoes in Fraunces italic, feminine forms (maestras); no em-dashes. Copy must stay verifiable: the hands-on-heart moments were **guided breathwork** (never "pledge"/oath), there was no stage and no dance floor, and the studio's train murals are irrelevant background.
- **Placements (Aug 2026):** homepage dark spotlight + first carousel card, links feature card (`#graduation-card`) + "Grad 2026" highlight bubble (`hl_grad.png`), events recap band (`id="graduation-2026"`), and the repurposed public `/graduation` page (RSVP form retired; page removed from the sitemap exclusion list in `astro.config.mjs`). Share kit lives at `public/images/graduation/share/` (story, carousel-1..3, post) with curated photos at `public/images/graduation/g01-g16.jpg`.
- **Exception to identity exclusivity:** on the `/graduation` page's closing invitation section, the Sweat Fest and Gala cards each carry their **full own identity** (Option F daylight card; gala dark-luxe card with `.gala-foil` allowed inside that card only): event cards on another page act as ambassadors of their own brand.
...
- Current desktop media rhythm (left/right/left/right/left/right, Sep 2026): gala atrium photo on the left, ABC7 Windy City Weekend player on the right (it took over the WGN9 slot on Sep 26, 2026), Monday Miles tee on the left, 40 Under 40 flyer on the right, Monday Miles visual on the left, and the Studio / Grand Opening image on the right. (The recap band, the events carousel, Instagram, and Press are card grids or feeds, not media spotlights, so they do not affect the zig-zag.)
...
**Page section order (Sep 2026: the live ticketed event leads, then recaps in reverse-chronological order):** profile header, language toggle, Highlight Stories bar, then the four event feature cards: the **Gala feature card** (`#gala-card`, Sep 25, first while full-evening tickets are on sale; it carries a gold `data-gala-days-left` chip, an EN/ES two-tile price spotlight from `galaTickets.tiers`, an **inline bilingual `GalaTicketDeadline` module** between the price tiles and the closing `statusLabel` line, and the `galaTeaser.statusLabel` sign-off; the module's gold CTA is the card's only ticket ask, so the old bottom "Get Your Full-Night Gala Ticket" button was removed), the **Chicago Reader feature card** (`#reader-card`, Aug 21 story), the **Graduation 2026 card** (`#graduation-card`, Aug 7 recap, Option H), and the **Sweat Fest feature card** (`#sweatfest-card`, Aug 22 recap), then the main Quick Links list, then newsletter and app-download sections. The Highlight Stories bar mirrors the same order: Gala 2026 -> Reader -> Grad 2026 -> Sweat Fest bubbles anchor to their cards, followed by the evergreen WGN, YTT 2025, Therapy, and App bubbles. Card `animation-delay` values are re-staggered to DOM order whenever the cards are reordered. **Sep 26, 2026:** the ABC7 Windy City Weekend card (`#abc7-card`, see the September 26 note in section 2) now sits between the gala card and the Reader card, with a "📺 ABC7" bubble second in the bar; the five cards run Gala -> ABC7 -> Reader -> Graduation -> Sweat Fest at 80, 95, 110, 125, 140 ms.

**Post-gala recap (Sep 26, 2026):** `#gala-card` keeps its first position (newest story), its black-tie stationery shell, and its `animation-delay: 80ms` stagger, but is now a bilingual recap card: an "Annual Gala 2026 / Gala Anual 2026" eyebrow with a gold "Recap / Recap" chip (replacing the `data-gala-days-left` chip), the date line, the local 4:5 recap photo with the foil "Gracias, comunidad" heading over it, the `galaRecapCard` EN blurb with its one-line ES counterpart, the three aggregate stats as small gold-keyline tiles (ES labels: "recaudados esa noche", "paletas levantadas", "fotos"), and ONE gold CTA "See the photos / Ver las fotos" -> `/gala#photos` (`gala_recap_click` / `links_gala_recap`). The price spotlight, the tier note, the inline `GalaTicketDeadline`, and the `galaDaysLeft*` frontmatter were removed. Every visible string carries a `.lang-en` / `.lang-es` pair. The "Gala 2026" highlight bubble still anchors to `#gala-card`.

**Sweat Fest feature card (bright treatment, July 2026):** restyled from its original dark-luxe treatment to the Option F bright daylight-festival identity so it visually contrasts the dark gala card directly above it: a cream gradient background (`from-[#fff8ef] via-white to-[#ffefe2]`) with a sunrise -> ember -> magenta `h-1` top strip, off-black text, the deep-stop gradient wordmark, sticker-style white ticket-tier tiles, the square art (`sweatFestArt.square.src`) in a soft warm frame, and a sunrise-gradient "Explore Sweat Fest" CTA with a hex-value drop shadow (`shadow-[#ff6b52]/25`, not `shadow-[var()]/NN`, which silently fails under Tailwind v4). Kept exactly as before: the card's id (`sweatfest-card`), `scroll-mt-24`, its position among the feature cards ahead of the Quick Links divider, its `sweatfest_explore_click` conversion attrs, and every one of its 14 EN/14 ES `lang-en`/`lang-es` span pairs (styling changed only; no copy was restructured). The Highlight Stories bar's "Sweat Fest" bubble still anchors to `#sweatfest-card`.
...
Current active event links (chronological): none at present in the `links` array itself (the **Celebrating Pride / Celebrando el Orgullo** card was removed in July 2026 once Pride Month passed, and the **YTT '26 Trio** card was deleted in September 2026 once that series ended). The Gala, Reader, Graduation, and Sweat Fest cards are not entries in this array; they are the four standalone feature cards documented above, rendered ahead of the Quick Links divider.
...
- **Branded Story Highlights Covers**: Uses custom, high-fidelity AI-generated covers stored in `public/images/highlights/hl_*.png` (Free Classes, WGN Feature, YTT, Therapy, App). Outlet bubbles (Reader, ABC7) use the outlet's own mark instead: `hl_reader.png` (wordmark on Reader yellow) and `hl_abc7.png` (the transparent ABC7 circle-7 logo, which sits on the bubble's dark fill).
...
## 🥂 8B. Gala Sales Page & Signature "Black-Tie Luxe" System (`src/pages/gala.astro`)

The public `/gala` page is LSP's **signature fundraising event** page, so it carries a more elevated, cinematic treatment than any other page while staying inside the core palette (gold `#ffbd59`, mauve `#b5a18d`, off-black `#1E1E1E`). Since July 2026 it is a full sales page: tickets and sponsorships are LIVE on Zeffy (`galaTeaser.ticketsUrl`) and every CTA drives there with `data-conversion-event="gala_ticket_click"` plus a per-placement `data-conversion-context`.

### Section order (narrative)
**Hero (`#save-the-date`) → Impact band (`#impact`) → Evening with purpose (MCA venue) → Tickets (`#tickets`: evening timeline + tier cards + sponsorship strip) → Last year recap (light intermission, with 2025 sponsor gratitude marquee) → Closing invitation (`#gala-interest`, id kept for legacy anchors).** The impact band sits high so the fundraising case lands before the venue; the tickets section follows the venue so desire peaks before the ask. Animated gold ribbon dividers separate the major dark sections, and a sticky Get-Tickets bar follows the reader through the middle of the page.

### Evening timeline + tickets section (`#tickets`)
The signature move carried over from the printed two-pager: the evening, hour by hour, with tier access drawn to scale.
- **Data**: `galaEvening` in `src/data/galaTeaser.js` (stops carry `time`, `title`, `detail`, `at` = 0..1 position along the 6 PM → midnight track, `tiers`); `galaTickets` holds tier prices/close dates and the three sponsorship packages. **Prices and dates are printed on physical collateral too — change them only in `galaTeaser.js` and keep them matching the Zeffy form.**
- **Desktop (lg+)**: an absolute-positioned horizontal track (`.gala-tl-track`, a `gala-foil-rule` drawing in via scaleX), diamond stops (`.gala-tl-pop`, staggered via `--tl-delay`), stop copy alternating above/below the line, faint hour ticks, and a traveling glow (`.gala-tl-glow`, `@keyframes gala-tl-travel`, motion-safe). Below the track sit two **access bars at true scale**: solid-gold Benefactor (6 PM → 12 AM, full width) and outlined Supporter (`w-1/2`, exactly the 9 PM → midnight half, `origin-left` draw-in), plus an italic sponsor-tables note. The bars visually anchor the tier cards below, whose "From 6 PM"/"From 9 PM" chips echo them.
- **Mobile (< lg)**: a vertical gold rail with diamond stops, a tier chip per stop ("Benefactor tier" / "All ticket tiers"), and stacked access chips.
- All initial-hidden animation states are gated behind `.js` and keyed off the standard `.reveal`/`.is-visible` observer with a reduced-motion escape hatch — the same contract as the rest of the page.
- Tier cards + sponsorship strip reuse the `gala-detail-card` corner-bracket treatment; every "Get Tickets"/"Become a Sponsor" button opens the Zeffy form in a new tab.
...
### Ticket sales (live Zeffy form; interest list retired)
- Tickets and sponsorships are live at `galaTeaser.ticketsUrl` (Zeffy). Every "Get Tickets"/"Become a Sponsor" CTA (gala hero, tickets section, closing card, sticky bar, homepage band, events featured, links card) links out to it in a new tab with `data-conversion-event="gala_ticket_click"` tracking and a per-placement context. The old pre-sales interest list (`galaTeaser.interestFormUrl`, event `gala_interest_start`) is retired: the field stays in `galaTeaser.js` for reference but no live CTA points at it. Since September 2026 `statusLabel` reads "Full-evening tickets close September 15" (it was "Tickets and sponsorships are on sale now") and a companion `lateNightStatusLabel` reads "Late-night access on sale through September 25"; both are rendered on the links card and inside the deadline module. The hero "Get Tickets" button scrolls to `#tickets` so prices land before the hand-off to Zeffy.
...
- **Floating gold particles**: a `pointer-events-none` layer of ~22 gold dots generated from a `heroParticles` frontmatter array, drifting up via `@keyframes gala-float-up`. A few larger "bokeh" motes (`.gala-particle--bokeh`) carry a wider soft halo for depth variation. Desktop only (`hidden lg:block`), invisible/static under reduced motion. A lighter `ambientParticles` field (~10 dots) repeats in the venue section and a denser `closingParticles` field (~16 dots) in the closing invitation so the page bookends visually.
- **Aurora backdrop** (`.gala-aurora`): a slow-drifting layered radial-gradient glow (gold + mauve, `@keyframes gala-aurora-drift`, ~18s) behind the hero grid; a softer `--soft` variant in the impact band for continuity. Motion-safe only.
- **Theater light sweep** (`.gala-theater-sweep`): a one-time diagonal highlight that crosses the hero on load (`@keyframes gala-sweep-once`, 1.6s, fires once). Motion-safe only.
- **Spotlight** (`.gala-spotlight`): a soft radial glow behind the framed hero photo that breathes via the existing `.gala-ambient` pulse.
- **Countdown panel glow** (`.gala-countdown-panel`): the countdown bar runs a subtle continuous border-glow loop (`@keyframes gala-panel-glow`, 4.5s) so it reads as the live focal point. The panel still uses `GalaCountdown variant="compact"`.
- **Gold ribbon dividers** (`.gala-ribbon`): shimmering `.gala-foil-rule` hairlines with a centered rotated diamond between major sections, giving a cohesive black-tie trim rhythm.
- **Hero photo parallax**: a `[data-gala-parallax]` wrapper around the hero `GalaPhotoFrame` (and its "Black tie" badge) gets a small rAF-throttled `translate3d` on scroll (clamped to 50px) for depth. Skipped entirely under reduced motion. The script is now **generalized**: it queries all `[data-gala-parallax]` elements, reads an optional per-element `data-gala-speed` (default `0.1`), and clamps drift proportionally — so subtle venue frames can carry a faint 0.04-speed layer.

### 2025 sponsor gratitude marquee (recap section)
Under the light-background recap section, a tasteful gratitude treatment rather than a sponsor pitch: a thin "With gratitude to our 2025 sponsors" caption followed by an infinite marquee (`.gala-marquee`, `@keyframes gala-marquee-left`, ~40s linear) of the 20 partner logos from `/public/images/galalogos/*.png` (excludes the recap photo and QR). Logos render **grayscale by default and gain color on hover** (`.gala-sponsor-logo`), pause on row hover, and the track is duplicated (two identical halves) for a seamless loop; the second half is `aria-hidden`. Under reduced motion the marquee falls back to a centered wrapping row. It closes with a subtle "Interested in 2026 sponsorship opportunities? See sponsorship packages →" text link to `#tickets` (`data-conversion-event="gala_ticket_click"`, context `gala_page_sponsor_tease`).

### Sticky Get-Tickets conversion bar
A slim `.gala-sticky-bar` fixed to the bottom of the viewport that fades/slides in (`translate-y-0 opacity-100` via the `.gala-sticky--visible` class) once the hero has scrolled away and hides again when the closing `#gala-interest` section reaches view (so it never doubles the final CTA). Contains the gala short title + short date/time and a compact "Get Tickets" button to the Zeffy form (`context gala_page_sticky`). It is session-dismissible via a close button and is `aria-hidden` until shown. Driven by two `IntersectionObserver`s (hero + closing sections).

### `GalaTicketDeadline` contract (September 2026)
`src/components/GalaTicketDeadline.astro` is the site's single gala conversion unit: every surface that asks for a ticket asks through it, so the countdown, the phase logic, and the copy can never drift between pages.

- **Props.** `context: string` (required; becomes the `data-conversion-context` on the primary CTA and seeds the derived contexts below), `bilingual?: boolean` (default false, EN only; when true it renders `.lang-en` / `.lang-es hidden` span pairs against the links page's `.lang-active-es` switch), `variant?: "banner" | "inline"` (default `banner`). **Banner** is the standalone card (rounded-3xl, `border-accent-gold/45`, `bg-off-black`, a `gala-foil-rule` hairline on the top edge, `.gala-lattice` at 0.07, one gold and one mauve `.gala-ambient` blob, container-query two-column layout that adds a `GalaPhotoFrame` of the facade at >= 64rem) used on `/schedule` and `/gala`. **Inline** is the module alone with a top hairline and no card chrome, dropped inside an existing dark gala section; it must stay clean at 320px wide, which is why the countdown is three tiles.
- **Phases and boundaries** come from `galaTicketDeadline` in `src/data/galaTeaser.js`: **full** before `closesAtISO` (`2026-09-16T00:00:00-05:00`, i.e. the end of Tuesday, September 15), **late** between that and `eventEndsAtISO` (`2026-09-26T00:00:00-05:00`, the exclusive end of gala day), **past** after it. All three blocks are always server-rendered with `data-phase="full|late|past"` and the inactive ones carry `hidden`; the server picks a fallback from `Date.now()` and the inline script decides for real on load and every 30 seconds. In `past` the whole root is hidden.
  - **full** shows the module: a small uppercase lede ("Full-evening seats close"), a large `font-display gala-foil` `data-gala-days-left` line, the quiet `closesDateLabel · closesTimeLabel` line, a `GalaCountdown` in days/hours/minutes, the gold CTA "Reserve the Full Evening · ${fullNightPrice}", the "choose Benefactor at checkout" hint, and the quiet late-night text link.
  - **late** swaps in "The full evening has closed. The night is still open." with a gold "Get Late-Night Access · ${lateNightPrice}" CTA and the `galaTeaser.lateNightStatusLabel` note (the inline variant compresses this to one line plus the CTA).
  - **past** hides everything and lets the site-wide CTA fallback take over.
- **Site-wide data hooks** (the script queries the whole document, not just the component, and runs once per page behind a window guard). Spanish is detected per element: `data-lang="es"`, a `.lang-es` class, or a `.lang-es` ancestor.
  - `[data-gala-days-left]`: "{n} days left" / "1 day left" / "Closes tonight"; late "Late-night access open"; past empty. ES: "Quedan {n} días" / "Queda 1 día" / "Cierra esta noche" / "Acceso nocturno disponible".
  - `[data-gala-closes-in]`: the sentence-fragment form for inline use in a pill, "closes in {n} days" / "closes in 1 day" / "closes tonight"; late "late-night access on sale"; past empty, with the matching ES strings.
  - `[data-gala-phase-only="full|late|past"]`: hidden unless the current phase matches. Available for one-off phase copy; no page uses it yet.
  - `[data-gala-full-night-cta]`: untouched in **full**; in **late** the href stays the Zeffy form (target, rel, and conversion attrs intact) and the label becomes "Get Late-Night Access · ${lateNightPrice}" / "Acceso nocturno · ${lateNightPrice}"; in **past** the href falls back to `/gala#tickets`, `target` and `data-conversion-event` are removed, and the label becomes "View Gala Details" / "Ver detalles de la gala". Relabeling always goes through the `[data-gala-cta-label]` span inside the link (then a bilingual pair inside it, then the link's first span), so arrow icons are never clobbered. Wrap the label text in that span on any new CTA.
- **Days-left formula** (use it verbatim on every server-rendered fallback so nothing reflows when the script runs): `hoursLeft = (closesAt - now) / 3.6e6`, then `days = Math.floor(hoursLeft / 24)`. A result of 0 renders "Closes tonight", never "0 days left". The homepage hero pill, the carousel chip, and the links chip each compute their fallback this way.
- **`GalaCountdown` `units` prop**: `units?: Array<"days"|"hours"|"minutes"|"seconds">` (default all four). The deadline module passes `["days","hours","minutes"]`, since dropping the seconds reads as a calm deadline rather than a ticking bomb. Column counts are written as literal `grid-cols-2/3/4` classes so Tailwind's scanner builds them.
- **Copy rules (binding for any future deadline surface).** One ask per surface: wherever the module lands, nothing else in that section repeats the ticket CTA (the homepage spotlight, the events featured section, and the links gala card each gave up their own Get Tickets button and kept only a ghost "Explore the Gala" link and a sponsorship text link). No `animate-ping` dots, no red, no caps-lock urgency: urgency reads through a static rotated gold diamond, the foil days-left line, and the countdown. The deadline date is stated **once** per surface. The $225 late-night option is offered as a quiet white/70 text link under the primary CTA, never as a competing button, so the full evening stays the ask.
- **Conversion contexts in use**: `home_gala_spotlight`, `events_featured_gala`, `links_gala_card`, `schedule`, `gala_page_deadline`. The component derives `{context}_late_night` for the quiet alternative link and `{context}_late` for the late-phase CTA, so each placement reports all three. The sponsorship teases keep their own contexts: `home_gala_sponsor_tease` and `events_gala_sponsor_tease`.

### Cross-site touchpoints (kept consistent)
The gala presence is elevated the same way everywhere, and since the September 2026 refresh each of these surfaces makes exactly **one** ask, through the shared deadline module: the **homepage** leads with the gala spotlight directly under the hero (foil serif title, corner-framed atrium photo on the left, a `galaEvening` timeline strip, the inline `GalaTicketDeadline`, a ghost "Explore the Gala" link, a sponsorship text link, and the `GalaImpactStats` strip), and its events-carousel card leads with a "Black tie" badge plus a gold `data-gala-days-left` chip; **`events.astro`** opens on the featured gala section (foil title, corner-framed photo, `galaEvening` chip row, inline module); **`schedule.astro`** carries the banner variant above the policy strip; and **`links.astro`** leads with the gala card (bilingual foil title, corner-framed photo, price spotlight, inline bilingual module, `statusLabel` sign-off; EN/ES spans and `links-slide-up` preserved).
...
- `project-brief-doc`: server-side port of the legacy Project Brief Apps Script. Given a `project_id`, copies the "Project Brief (Template)" Google Doc into the team Drive folder (Shared Drive), names it `YYYY-MM-DD_<Title>`, and writes the link onto `projects.details_url`. Idempotent (skips when `details_url` is set unless `force:true`). Authenticates to Google with a service account (`GOOGLE_SERVICE_ACCOUNT_JSON`, `BRIEF_DOC_TEMPLATE_ID`, `BRIEF_DOC_FOLDER_ID` secrets); callers present either the `x-brief-doc-secret` header, the service role key, or an admin/superuser user JWT.

### Project Brief Doc Automation
When a project is created (manual create or Google Form intake), an `AFTER INSERT` trigger on `public.projects` (`app_private.enqueue_brief_doc`) calls the `project-brief-doc` function via `pg_net` so a brief Doc is auto-created, mirroring the old Sheets `onEdit` automation. There is also a **Generate brief doc** button in `ProjectDetailDrawer.svelte` for on-demand/fallback creation, available to any marketing-module member (the edge function authorizes the secret header, a service-role token, or a signed-in admin/superuser **or marketing-module member** JWT). Plumbing lives in `app_private.settings` (`brief_doc_function_url`, `brief_doc_secret`); the trigger stays dormant until the URL is set. `projects.brief_doc_status` tracks state (`pending`/`created`/`error`). Bulk imports opt out with `set local lsp.skip_brief_automation = 'on';`. Full provisioning steps (Shared Drive, service account, secrets, deploy order) are in `docs/supabase/project-brief-doc-setup.md`. Migration: `20260622000100_project_brief_doc_automation.sql`.

**Brief lifecycle UX.** A `BEFORE INSERT` trigger (`app_private.mark_brief_doc_pending`) marks auto-brief-eligible rows `brief_doc_status='pending'` so the UI is accurate; the `AFTER INSERT` enqueue fires only for `pending` rows. The project drawer's **Project brief** section renders three states off `details_url` + `brief_doc_status`: a clickable **Open brief doc** link when set; a **Generating brief doc…** state (with a short client-side poll, and a "Check now") while `pending`; otherwise **Generate brief doc** (or **Retry** after `error`). This is why the brief link appears without a manual refresh. The generated link lives in `details_url` (shown as "Open brief doc" and under External links → Details), NOT `deliverables_url` (which stays for separately-supplied deliverable assets). Migration: `20260622010000_brief_doc_pending_status.sql`. `brief_doc_status` is included in the project select columns in `DashboardApp.svelte`, `ProjectDetailDrawer.svelte`, and `ProjectCreateDrawer.svelte`.

**Member project creation.** Creating projects is not admin-only. The projects RLS INSERT policy is gated on `has_module('marketing')`, so any marketing-module member can create. The shared `ProjectCreateDrawer.svelte` (a member-safe create drawer: `copy_approved` stays false, `source='manual'`, `intake_reviewed=true`) is opened from a **New project** button on the **Kanban** board header and the **Workspace** header (wired in `DashboardApp.svelte`, which also passes `onCreateProject` into `Workspace.svelte`). New projects get their brief Doc via the same auto trigger. The Admin Overview keeps its own fuller admin create drawer (with admin-only fields like `copy_approved`/intake state).
