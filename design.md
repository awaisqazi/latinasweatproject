...
Use this for celebration events or cultural observances where the moment carries its own palette but the site still needs LSP's warm, premium look. It is **currently carried by Celebrating Pride** (standalone `/pride` page + homepage lead spotlight + homepage Pride Teacher Spotlight + events featured section + links card, final Pride weekend, June 2026). It was previously carried by **Honoring Juneteenth** (passed June 21, 2026, retired from all placements; the standalone `/juneteenth` page remains live but unlinked) and before that by **Pride in the Park** (passed June 14, 2026). Keep it on hand for the next moment that ships its own palette.
...
- **Dual-cause site banner:** during overlapping observances, `src/components/PrideTheme.astro` can run the banner as **two cross-fading slides** so neither cause is subordinated (this carried Pride + Juneteenth simultaneously in mid-June 2026). Keep observance banners date-gated and self-removing rather than relying on a manual takedown.
