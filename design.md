...
graph TD
    ...
    Sec1 --> Sec2["Section 2: Haus of LSP Summer Renaissance (Dark Plum, Option G)"]
    Sec2 --> Sec3["Section 3: Merch Drop (Warm Amber)"]
    Sec3 --> Sec4["Section 4: Noche Inolvidable Gala (bg-off-black)"]
    Sec4 --> Sec5["Section 5: Free Classes Promo (Light Green/Sand)"]
    Sec5 --> Sec6["Section 6: WGN9 Spotlight (bg-off-black)"]
    Sec6 --> Sec7["Section 7: Monday Miles (Light Sand)"]
    Sec7 --> Sec8["Section 8: 40 Under 40 (bg-off-black)"]
    Sec8 --> Sec9["Section 9: Events Carousel (Light Sand)"]
    Sec9 --> Sec10["Section 10: Studio / Grand Opening (bg-off-black)"]
    Sec10 --> Sec11["Section 11: Press (Light)"]
    Sec11 --> Sec12["Section 12: Instagram (bg-off-black)"]
```
...
**Page section order (feature cards run chronologically):** profile header, language toggle, Highlight Stories bar, then the three event feature cards in date order: the **Haus of LSP Summer Renaissance card** (`#hausoflsp-card`, Aug 1, Option G dark collage treatment with a violet offset shadow, added July 2026), the **Sweat Fest feature card** (`#sweatfest-card`, Aug 22), and the **Gala feature card** (Sep 25; it carries an "On Sale Now" / "A la Venta" badge, an EN/ES two-tile price spotlight from `galaTickets.tiers`, and a gold "Get Tickets" CTA straight to the live Zeffy form with `gala_ticket_click` tracking), then the main Quick Links list, then the Free Classes two-locations promo, newsletter, and app-download sections. The Highlight Stories bar mirrors the same chronology: Haus of LSP (`hl_haus.png`) -> Sweat Fest -> Gala 2026 bubbles anchor to their cards.
...
- Tickets and sponsorships are live at `galaTeaser.ticketsUrl` (Zeffy). Every "Get Tickets"/"Become a Sponsor" CTA (gala hero, tickets section, closing card, sticky bar, homepage band, events featured, links card) links out to it in a new tab with `data-conversion-event="gala_ticket_click"` tracking and a per-placement context. The old pre-sales interest list (`galaTeaser.interestFormUrl`, event `gala_interest_start`) is retired: the field stays in `galaTeaser.js` for reference but no live CTA points at it. `statusLabel` reads "Tickets and sponsorships are on sale now". The hero "Get Tickets" button scrolls to `#tickets` so prices land before the hand-off to Zeffy.
...
The gala presence is elevated the same way everywhere: **homepage** gala band (foil serif title + `GalaImpactStats` strip + "Get Tickets" primary CTA + early-bird note) and events-carousel card ("On Sale Now" chip), **`events.astro`** featured section (foil title, corner-framed photo, "Get Tickets" primary CTA + early-bird note), and **`links.astro`** gala card (leads the page while sales run; foil bilingual title, corner-framed photo, price spotlight, sheening "Get Tickets" bar; EN/ES spans and `links-slide-up` preserved).
