...
> [!NOTE]
> **August 2026: Summer Renaissance retired, Graduation 2026 added (same-theme slot swaps, zero reordering).** Summer Renaissance passed Aug 1, so its homepage section, carousel card, events featured section, links card, and highlight bubble were all retired (the `/hausoflsp` page persists, per the Pride precedent). Every vacated slot was refilled by the **Graduation 2026** recap (Option H below) in the SAME theme, so both pages keep their documented cadence untouched: homepage runs Hero D -> Sweat Fest L -> **Graduation D** -> Merch L -> Gala D -> Free Day L -> WGN9 D -> Monday Miles L -> 40 Under 40 D -> Carousel L -> Grand Opening D -> Press L -> Instagram D, with the graduation media on the RIGHT in the zig-zag (as the Haus collage was); the events page runs Header L -> **Graduation recap D** (`id="graduation-2026"`) -> Sweat Fest L -> Gala D -> EventsList L -> Monday Miles L -> Stay Connected L. The events carousel gained a "Graduation 2026 Gallery" card in first position; links feature cards run Graduation -> Sweat Fest -> Gala (still chronological: Aug 7 recap, Aug 22, Sep 25) with a "Grad 2026" bubble first in the highlights bar. Lesson reaffirmed: refilling a passed event's slot with a same-theme section is far lower risk than reordering.
